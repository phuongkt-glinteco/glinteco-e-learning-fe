...
export class UpdateTracksAndLessonsSchema1784400000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_tracks_name_search"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tracks" ADD "description" text NOT NULL DEFAULT ''`,
    );
    await queryRunner.query(
      `ALTER TABLE "tracks" ADD "estimatedTime" character varying NOT NULL DEFAULT '1h'`,
    );
    await queryRunner.query(
      `ALTER TABLE "tracks" ADD "icon" character varying NOT NULL DEFAULT 'flag'`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "lessons" RENAME COLUMN "name" TO "title"`,
    );
    await queryRunner.query(
      `ALTER TABLE "lessons" ADD "estimatedTime" character varying NOT NULL DEFAULT '30m'`,
    );
    await queryRunner.query(
      `ALTER TABLE "lessons" RENAME COLUMN "content" TO "body"`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "lessons" RENAME COLUMN "body" TO "content"`,
    );
    await queryRunner.query(
      `ALTER TABLE "lessons" DROP COLUMN "estimatedTime"`,
    );
    await queryRunner.query(
      `ALTER TABLE "lessons" RENAME COLUMN "title" TO "name"`,
    );

    // 2. Drop index on title column
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_tracks_title_search"`,
    );
