  ...
  try {
    ...
    await lessonRepo.save([
      ...
      lessonRepo.create({
        ...
        order: 1,
        content: 'Clone the git monorepo locally and checkout your branch.',
      }),
      lessonRepo.create({
        trackId: t1.id,
        name: 'Install pnpm & Tooling',
        order: 2,
        content: 'Install node, pnpm and configure workspace dependencies.',
      }),
      lessonRepo.create({
        trackId: t1.id,
        name: 'Running with Docker Compose',
        order: 3,
        content: 'Run postgres and redis containers locally via compose.',
      }),
      lessonRepo.create({
        trackId: t1.id,
        name: 'Verification Checkpoint',
        order: 4,
        content: 'Verify all services are up and you can load the app locally.',
      }),
      ...
      lessonRepo.create({
        trackId: t2.id,
        name: 'Tailoring Tailwind CSS',
        order: 2,
        content: 'Configure config, utilities, and custom spacing systems.',
      }),
      lessonRepo.create({
        trackId: t2.id,
        name: 'Shared Component Library',
        order: 3,
        content: 'Import and configure global design tokens and assets.',
      }),
      lessonRepo.create({
        trackId: t2.id,
        name: 'Accessibility Standards',
        order: 4,
        content: 'ARIA landmarks, screen readers, contrast, and focus states.',
      }),
      lessonRepo.create({
        trackId: t2.id,
        name: 'Design Tokens & Layouts',
        order: 5,
        content: 'Apply premium margins, sizing constraints, and dynamic tokens.',
      }),
      lessonRepo.create({
        trackId: t2.id,
        name: 'Building Reusable React Components',
        order: 6,
        content: 'Implement composite widgets using premium clean design codes.',
      }),

      // T3
      lessonRepo.create({
        trackId: t3.id,
        name: 'Modules & Providers in NestJS',
        order: 1,
        content: 'Configure dependency injection, module boundaries and imports.',
      }),
      lessonRepo.create({
        trackId: t3.id,
        name: 'Controller & Routing Layers',
        order: 2,
        content: 'Write REST endpoints with Swagger docs and HTTP status codes.',
      }),
      lessonRepo.create({
        trackId: t3.id,
        name: 'Request DTOs & Validation',
        order: 3,
        content: 'Apply class-validator constraints and DTO validation rules.',
      }),
      lessonRepo.create({
        trackId: t3.id,
        name: 'Database Integration with TypeORM',
        order: 4,
        content: 'Map relations, specify repositories and create entity migrations.',
      }),
      lessonRepo.create({
        trackId: t3.id,
        name: 'Pagination & Service Methods',
        order: 5,
        content: 'Implement keyset cursor pagination to deliver stable feeds.',
      }),

      // T4
      lessonRepo.create({
        trackId: t4.id,
        name: 'Event-Driven Boundaries',
        order: 1,
        content: 'Implement event-emitters to decouple notification pipelines.',
      }),
      lessonRepo.create({
        trackId: t4.id,
        name: 'Gateway Pattern Setup',
        order: 2,
        content: 'Structure clean API proxies with reverse proxy services.',
      }),
      lessonRepo.create({
        trackId: t4.id,
        name: 'Contract Definitions',
        order: 3,
        content: 'Establish type contracts for events and service pipelines.',
      }),
      lessonRepo.create({
        trackId: t4.id,
        name: 'Order Flow Sequence',
        order: 4,
        content: 'Map events from checkout up to package fulfillment.',
      }),
      ...
      lessonRepo.create({
        ...
        order: 1,
        content: 'Mock repositories and verify service method logic.',
      }),
      lessonRepo.create({
        trackId: t5.id,
        name: 'Integration test setup',
        order: 2,
        content: 'Write supertest specifications targeting controller routes.',
      }),
      lessonRepo.create({
        trackId: t5.id,
        name: 'Playwright and CI pipelines',
        order: 3,
        content: 'Automate browser tests and check status gate thresholds.',
      }),
    ]);
