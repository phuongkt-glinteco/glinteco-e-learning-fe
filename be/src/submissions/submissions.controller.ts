...
export class SubmissionsController {
  ...
  async submit(
    ...
    const saved = await this.submissionsService.submit(
      exerciseId,
      req.user.id,
      dto.prUrl,
    );
    const detailed = await this.submissionsService.findOne(
      saved.id,
      req.user.id,
      req.user.role,
    );
  ...
  @ApiResponse({
    status: 200,
    type: SubmissionDetailDto,
    description: 'Nộp lại bài tập thành công.',
  })
  ...
  async resubmit(
    ...
    const saved = await this.submissionsService.resubmit(
      exerciseId,
      req.user.id,
      dto.prUrl,
    );
  ...
  @ApiResponse({
    ...
    description: 'Lấy hàng chờ thành công.',
  })
  ...
  @ApiResponse({
    status: 200,
    type: SubmissionDetailDto,
    description: 'Lấy chi tiết bài nộp thành công.',
  })
  @Get('submissions/:id')
  async findOne(
    @Param('id') id: string,
  ...
  @ApiResponse({
    status: 200,
    type: SubmissionDetailDto,
    description: 'Phê duyệt bài nộp thành công.',
  })
  @Roles(UserRole.ADMIN)
  ...
  @ApiResponse({
    status: 200,
    type: SubmissionDetailDto,
    description: 'Yêu cầu sửa đổi thành công.',
  })
  @Roles(UserRole.ADMIN)
  ...
  @ApiResponse({
    status: 200,
    type: SubmissionHistoryResponseDto,
  ...
  async findHistory(
    ...
    const result = await this.submissionsService.findHistory(
      id,
      req.user.id,
      req.user.role,
    );
