  ...
  if (!item) {
    return (
      <div className="flex flex-col gap-2">
        <p className="text-[14px] font-medium text-outline-variant uppercase tracking-wider">{label}</p>
        <div className="flex items-center gap-3 p-3 bg-surface-container-lowest rounded-lg border border-outline-variant opacity-50">
          ...
          <div className="flex-1">
            <p className="text-[14px] font-medium text-on-surface-variant">{t('noPrevNextTrack')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        <div className="flex-1">
          <p className={`text-[14px] font-medium ${isLocked ? 'text-on-surface-variant' : 'text-on-surface'}`}>
            {item.title}
          </p>
          <p
    ...
    <Card className="border-outline-variant shadow-sm">
      <CardContent className="p-6 flex flex-col gap-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h3 className="text-[18px] font-semibold text-on-surface">{t('trackStatusTitle')}</h3>
        </div>
        <div className="flex flex-col gap-4">
          <PrevNextItem item={prevTrack} label={t('previousLabel')} />
          <div className="flex flex-col gap-2">
            <p className="text-[14px] font-medium text-outline-variant uppercase tracking-wider">{t('currentLabel')}</p>
            <div className="p-4 bg-primary/5 rounded-xl border border-primary/10">
              <div className="flex justify-between items-center mb-2">
                <p className="text-[14px] font-medium text-primary">{currentTitle}</p>
                <p className="text-[14px] font-medium text-primary font-bold">{progress}%</p>
              </div>
              <div className="w-full h-1.5 bg-primary/10 rounded-full overflow-hidden">
                <div className="h-full bg-primary rounded-full" style={{ width: `${progress}%` }}></div>
              </div>
              <div className="mt-3 flex items-center justify-between">
                <span className="text-[14px] font-medium text-secondary">{t('potentialRewardLabel')}</span>
                <span className="text-[14px] font-medium text-secondary">{totalXP.toLocaleString()} XP</span>
              </div>
            </div>
          </div>
